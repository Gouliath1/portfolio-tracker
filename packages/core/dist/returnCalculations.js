 ...
 */
export const calculateAnnualizedReturn = (totalReturn, startDate) => {
...
export const calculatePortfolioCagrSinceInception = (summary) => {
    ...
    if (years <= 0)
        return null;
    if (summary.totalCostJPY <= 0 || summary.totalValueJPY <= 0)
        return null;
...
export const calculatePortfolioAnnualizedReturn = (summary) => {
    if (summary.positions.length === 0)
        return null;
    const cashflows = [];
    let earliest = null;
    for (const p of summary.positions) {
        if (!p.transactionDate || !isFinite(p.costInJPY))
            continue;
    ...
    const toYears = (d) => (d.getTime() - earliest.getTime()) / (1000 * 60 * 60 * 24 * 365.25);
    ...
    if (totalOut === 0 || summary.totalValueJPY <= 0)
        return null;
    // XIRR via Newton-Raphson with fallback bisection
    const npv = (r) => xs.reduce((s, x) => s + x.a / Math.pow(1 + r, x.t), 0);
    const dnpv = (r) => xs.reduce((s, x) => s - (x.t * x.a) / Math.pow(1 + r, x.t + 1), 0);
    let rate = 0.1; // 10% initial guess
    let ok = false;
    for (let i = 0; i < 50; i++) {
        const f = npv(rate);
        const fp = dnpv(rate);
        if (!isFinite(fp) || Math.abs(fp) < 1e-12)
            break;
...
function calculateWeightedAverageFallback(summary) {
