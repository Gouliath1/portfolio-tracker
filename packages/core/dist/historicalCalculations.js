export function generateMonthlyDates(startDate, endDate = new Date()) {
    const dates = [];
...
export function getPortfolioStateAtDate(positions, targetDate) {
    ...
    relevantPositions.forEach(pos => {
        ...
        if (existing) {
            ...
            groupedPositions.set(key, Object.assign(Object.assign({}, pos), { quantity: newQuantity, costPerUnit: averageCost }));
        }
        else {
            groupedPositions.set(key, Object.assign({}, pos));
        }
    });
    const consolidatedPositions = Array.from(groupedPositions.values());
    const totalCost = consolidatedPositions.reduce((sum, pos) => sum + (pos.quantity * pos.costPerUnit), // Remove FX rate calculation for now
    0);
    return {
...
export function calculateHistoricalSnapshots(positions, startDate = new Date(Math.min(...positions.map(p => new Date(p.transactionDate).getTime())))) {
    const dates = generateMonthlyDates(startDate);
    return dates.map(date => getPortfolioStateAtDate(positions, date));
}
// eslint-disable-next-line @typescript-eslint/no-unused-vars
export async function fetchHistoricalPrices(symbol, dates) {
...
export async function calculateHistoricalValues(positions) {
    ...
    for (const snapshot of snapshots) {
        ...
        await Promise.all(snapshot.positions.map(async (pos) => {
