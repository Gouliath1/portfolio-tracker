...
async function getHistoricalFxRate(fxPair, date) {
...
async function convertCurrency(amount, fromCcy, toCcy, isHistorical = false, transactionDate) {
    if (fromCcy === toCcy) {
...
export const calculatePosition = async (rawPosition, currentPrice) => {
    ...
    if (rawPosition.transactionCcy !== rawPosition.stockCcy) {
        ...
        if (rawPosition.transactionCcy !== baseCcy) {
            const conversion = await convertCurrency(rawPosition.costPerUnit, rawPosition.transactionCcy, baseCcy, true, // historical
    ...
    if (currentPrice !== null) {
        if (rawPosition.stockCcy !== baseCcy) {
            ...
            const valueConversion = await convertCurrency(rawPosition.quantity * currentPrice, rawPosition.stockCcy, baseCcy, false // current rates
            );
    ...
    return Object.assign(Object.assign({}, rawPosition), { currentPrice, // Current price in stock currency
        costInJPY,
        currentValueJPY,
        pnlJPY,
        pnlPercentage, transactionFxRate: origFxRate, // Original FX rate (transaction ccy to base ccy)
        currentFxRate // Current FX rate (stock ccy to base ccy)
     });
};
export const calculatePortfolioSummary = async (rawPositions, forceRefresh = false) => {
    ...
    const positionPromises = rawPositions.map(pos => calculatePosition(pos, currentPrices[pos.ticker.toString()]));
