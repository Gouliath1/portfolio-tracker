...
import { createPositionSet, getActivePositionSet, } from '../database/operations/positionSetOperations';
...
const normalizeBrokerKey = (brokerName) => brokerName.toLowerCase().replace(/[^a-z0-9]+/g, '_');
...
const ensureActivePositionSet = async () => {
...
const ensureBroker = async (brokerName, preferredCurrency) => {
    const db = getDbClient();
    const displayName = (brokerName === null || brokerName === void 0 ? void 0 : brokerName.trim()) || 'Unknown Broker';
...
const ensureAccount = async (accountName, brokerId, baseCurrency) => {
    const db = getDbClient();
    const normalizedName = (accountName === null || accountName === void 0 ? void 0 : accountName.trim()) || 'General';
    const existing = await db.execute({
...
const ensureSecurity = async (position) => {
    const db = getDbClient();
...
const insertPositionRecord = async (positionSetId, accountId, securityId, position) => {
    const db = getDbClient();
...
const normalizePositionsInput = (data) => {
...
export const readPositionsFromFile = async (filePath = getPositionsFilePath()) => {
    const fs = loadFs();
    if (!fs) {
...
export const upsertPositionsForSet = async (positionSetId, positions, options = {}) => {
...
export const replaceActivePositionSetPositions = async (positions) => {
...
export const importPositionsFromFile = async (filePath = getPositionsFilePath()) => {
    const positions = await readPositionsFromFile(filePath);
    const count = await replaceActivePositionSetPositions(positions);
    return { count, positions };
};
export const getPositionsFileStatus = async (filePath = getPositionsFilePath()) => {
    try {
        const positions = await readPositionsFromFile(filePath);
        return {
            hasFile: true,
...
export const writePositionsFile = async (positions, filePath = getPositionsFilePath()) => {
    const fs = loadFs();
    const path = loadPath();
    if (!fs || !path) {
