 ...
 */
export async function getAllPositionSets() {
 ...
 */
export async function getActivePositionSet() {
    const client = getDbClient();
    const result = await client.execute(`
        SELECT * FROM position_sets 
 ...
 */
export async function createPositionSet(data) {
    const client = getDbClient();
 ...
 */
export async function setActivePositionSet(positionSetId) {
    const client = getDbClient();
 ...
 */
export async function isUsingDemoData() {
    const activeSet = await getActivePositionSet();
    return (activeSet === null || activeSet === void 0 ? void 0 : activeSet.info_type) === 'warning';
}
/**
 * Get position count for a specific position set
 */
export async function getPositionSetPositionCount(positionSetId) {
    const client = getDbClient();
    const result = await client.execute({
 ...
 */
export async function deletePositionSet(positionSetId) {
    const client = getDbClient();
    ...
    if (positionSet === null || positionSet === void 0 ? void 0 : positionSet.is_active) {
