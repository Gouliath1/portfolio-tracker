 ...
 */
export async function getFxRate(fxPair, requestedDate) {
 ...
 */
export async function storeFxRate(fxPair, rate, rateDate) {
    const db = await getDbClient();
    // Parse currency pair
    if (fxPair.length !== 6) {
 ...
 */
export async function getAllFxRatesForPair(fxPair) {
    const db = await getDbClient();
    if (fxPair.length !== 6) {
        return {};
    }
 ...
 */
export async function migrateFxRatesFromJson(jsonData) {
