                ...
                <tbody className="bg-white divide-y divide-gray-200">
                    {positions.map((position, index) => (
                        <tr key={`${position.ticker}-${index}`} className={index % 2 === 0 ? 'bg-white' : 'bg-gray-50'}>
                            ...
                            <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">¥{position.currentValueJPY.toLocaleString()}</td>
                            <td className={`px-6 py-4 whitespace-nowrap text-sm ${position.pnlJPY >= 0 ? 'text-green-600' : 'text-red-600'}`}>
                                ¥{position.pnlJPY.toLocaleString()}
                            </td>
                            <td className={`px-6 py-4 whitespace-nowrap text-sm ${position.pnlPercentage >= 0 ? 'text-green-600' : 'text-red-600'}`}>
